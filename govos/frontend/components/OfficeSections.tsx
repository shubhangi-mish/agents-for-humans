    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      ...
      {chain.length === 0 ? (
        <div style={{ fontSize: 11.5, color: "#4a4a4a", fontStyle: "italic", padding: "8px 2px" }}>
          Identifying which offices own this area…
        </div>
      ) : (
              ...
              >
                ...
                >
                  ...
                  <div style={{ position: "absolute", top: 8, right: 10, textAlign: "right" }}>
                    <div style={{ fontSize: 10.5, fontWeight: 800, color: "#eee" }}>{node.officer}</div>
                    <div style={{ fontSize: 9, color: "#888" }}>{node.role}</div>
                  </div>
                </div>
                    ...
                    >
                      {prev?.message && (
                        <div style={{ fontSize: 9.5, color: "#666", borderBottom: "1px solid #232323", paddingBottom: 5, marginBottom: 5 }}>
                          ↳ fwd from <strong style={{ color: "#888" }}>{prev.officer}</strong>: “{truncate(prev.message, 70)}”
                        </div>
                      )}
                      <div style={{ fontSize: 10.5, color: "#ccc", lineHeight: 1.45 }}>
                        <span style={{ color: "#666" }}>✉️ </span>
                        {truncate(node.message, 170)}
                      </div>
                    </motion.div>
                  ) : (
